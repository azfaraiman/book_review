...
if (isset($_GET['book_id'])) {
    if (isset($_POST['name'], $_POST['rating'], $_POST['content'])) {
        ...
        $stmt = $pdo->prepare('INSERT INTO reviews (book_id, name, content, rating, submit_date) VALUES (?,?,?,?,NOW())');
        $stmt->execute([$_GET['book_id'], $_POST['name'], $_POST['content'], $_POST['rating']]);
        exit('Your review has been submitted!');
    }
    // Get all reviews by the Page ID ordered by the submit date
    $stmt = $pdo->prepare('SELECT * FROM reviews WHERE book_id = ? ORDER BY submit_date DESC');
    $stmt->execute([$_GET['book_id']]);
    $reviews = $stmt->fetchAll(PDO::FETCH_ASSOC);
    // Get the overall rating and total amount of reviews
    $stmt = $pdo->prepare('SELECT AVG(rating) AS overall_rating, COUNT(*) AS total_reviews FROM reviews WHERE book_id = ?');
    $stmt->execute([$_GET['book_id']]);
    $reviews_info = $stmt->fetch(PDO::FETCH_ASSOC);
} else {
    exit('Please provide the book ID.');
}
?>
...
<div class="write_review">
    <form>
        ...
        <button onclick='window.location.reload(true);' type="submit">Submit Review</button>
    </form>
</div>
